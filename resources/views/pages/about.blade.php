<x-layouts.app>
    @php
        $story = $sections->get('story')?->data['body'] ?? 'We build company platforms that feel directed, premium, and cohesive rather than assembled from interchangeable content blocks.';
        $mission = $sections->get('mission')?->data['body'] ?? 'Translate business clarity into a stronger digital presence with structure that stays maintainable under real-world use.';
        $vision = $sections->get('vision')?->data['body'] ?? 'Create modern company websites that feel authored like a brand experience while remaining easy to manage operationally.';
        $values = $sections->get('values')?->data['items'] ?? [
    ...
    <section class="site-container section-space" data-reveal>
        <div class="grid gap-8 xl:grid-cols-[1.1fr_0.9fr] xl:items-end">
            ...
            <div class="glass-panel hover-glow p-6 sm:p-8">
    ...
    <section class="site-container section-space" data-reveal>
        ...
        <div class="grid gap-6 md:grid-cols-2 xl:grid-cols-3" data-reveal data-reveal-stagger="true">
